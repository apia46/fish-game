...
func _process(delta: float) -> void:
	...
	if !has_state(STATE_NONPROGRESS):
		...
		if bar.player.collision in %collision.get_overlapping_areas():
			...
			time_unhooked += delta
	game.progress_bar.value = (time_hooked - time_unhooked) / 60
	game.accuracy_label.text = "Accuracy: %.1f%%" % ((1 - time_unhooked/time) * 100)

func create_timer(state:int, duration:float, reciever:Callable, type=Timer) -> Timer:
...
func create_oneshot_timer(state:int, duration:float, reciever:Callable, type=Timer) -> Timer:
	var timer:Timer = create_timer(state, duration, reciever, type)
	timer.one_shot = true
	return timer
