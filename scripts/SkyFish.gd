...
func new_target_height() -> float:
	if phase == 0: return randf_range(bar.size.y-600+HALF_HEIGHT,bar.size.y-HALF_HEIGHT-50)
	else: return randf_range(max(HALF_HEIGHT+100, position.y-500), min(bar.size.y-HALF_HEIGHT, position.y+500))
