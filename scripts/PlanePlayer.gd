...
var cutscene_velocity:float = 0
var cutscene_direction:float = 0
...
func _process(delta: float) -> void:
	...
	if cutscene:
		...
		if velocity.length_squared() < 0.01:
			position += Vector2(cutscene_velocity, 0).rotated(cutscene_direction) * delta
		else:
	...
	if !cutscene or velocity.length_squared() >= 0.01:
		%sprite.rotation = velocity.y * 0.002 + rotation_offset
	else:
		%sprite.rotation = cutscene_direction

func _input(event: InputEvent) -> void:
	if !active or cutscene: return
